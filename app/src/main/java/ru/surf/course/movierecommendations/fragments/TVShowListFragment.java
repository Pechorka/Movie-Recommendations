...
import ru.surf.course.movierecommendations.adapters.GridTVShowsAdapter;
...
public class TVShowListFragment extends Fragment implements GetTVShowsTask.TaskCompletedListener {
    ...
    private GridTVShowsAdapter gridTVShowsAdapter;
    ...
    public static TVShowListFragment newInstance(String query, String language, Tasks task) {
    ...
    public static TVShowListFragment newInstance(int id, String language, Tasks task) {
        TVShowListFragment TVShowListFragment = new TVShowListFragment();
        Bundle bundle = new Bundle();
        bundle.putString(KEY_LANGUAGE, language);
    ...
    @Override
    public void tvshowsLoaded(List<TVShowInfo> result, boolean newResult) {
        if (result != null) {
            if (tvShowInfoList != null && !newResult) {
                tvShowInfoList.addAll(result);
            } else {
                tvShowInfoList = result;
            }
            dataLoadComplete();
        }
    }
    ...
    private void initViews(View root) {
        ...
        gridTVShowsAdapter = new GridTVShowsAdapter(getActivity(), new ArrayList<TVShowInfo>(1));
...
//        recyclerView.setAdapter(listMoviesAdapter);
    ...
    private void switchToGrid() {
        recyclerView.setAdapter(gridTVShowsAdapter);
    ...
    private void loadInformation(Tasks task) {
        switch (task) {
            case SEARCH_BY_FILTER:
                loadTVShowInfoByFilter(query, language, String.valueOf(PAGE));
                break;
...
//                loadMoviesInfoById(id,language,task);
    ...
    public void loadTVShowInfoByFilter(String query, String language, String page) {
        GetTVShowsTask getTVShowsTask = new GetTVShowsTask();
        getTVShowsTask.addListener(this);
        getTVShowsTask.getTVShowsByFilter(query, language, page);
    }
...
//    }
    ...
    public void fillInformation() {
        gridTVShowsAdapter.setTvShowInfoList(tvShowInfoList);
////        listMoviesAdapter.setMovieInfoList(movieInfoList);
        gridTVShowsAdapter.notifyDataSetChanged();

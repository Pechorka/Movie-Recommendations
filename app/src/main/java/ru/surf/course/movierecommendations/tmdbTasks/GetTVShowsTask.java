...
public class GetTVShowsTask extends GetMediaTask {
    ...
    if (isLoadingList) {
      ...
      for (int i = 0; i < tvShowArray.length(); i++) {
        ...
        item.setOriginalLanguage(
            new Locale(tvShowArray.getJSONObject(i).getString(TMDB_ORIGINAL_LANGUAGE)));
        try {
          item.setDate(
              formatter.parse(tvShowArray.getJSONObject(i).getString(TMDB_FIRST_AIR_DATE)));
  ...
  private Uri uriById(String id, String language, String page) {
    final String TMDB_BASE_URL = "https://api.themoviedb.org/3/tv/" + id + "?";
    return Uri.parse(TMDB_BASE_URL).buildUpon()
        .appendQueryParameter(API_KEY_PARAM, BuildConfig.TMDB_API_KEY)
        .appendQueryParameter(LANGUAGE_PARAM, language)
        .appendQueryParameter(PAGE_PARAM, page)

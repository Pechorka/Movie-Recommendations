...
public class MainActivityPresenter extends BasePresenter<MainActivityView> {
  ...
  private void setupRecommendationsItem() {
    if (getRecommendedGenreIds(MediaType.movie).length() == 0) {
      getView().deactivateRecommendationsMenuItem();
    }
  }
  ...
  private void initFragments(Tasks task) {
    movieListFragment = MediaListFragmentView
        .newInstance(query, region, task, Media.MediaType.movie);
  ...
  private void initFragments(Tasks task, String movieQuery, String tvshowQuery) {
    movieListFragment = MediaListFragmentView
        .newInstance(movieQuery, region, task, Media.MediaType.movie);
    tvShowListFragment = MediaListFragmentView
        .newInstance(tvshowQuery, region, task, Media.MediaType.tv);
  }
  ...
  private void loadInformationBySearchQuery() {
    ((MediaListFragmentPresenter) movieListFragment.getPresenter())
        .loadMediaByName(query, Utilities.getSystemLanguage(), "1", true);
    ((MediaListFragmentPresenter) tvShowListFragment.getPresenter())
        .loadMediaByName(query, Utilities.getSystemLanguage(), "1", true);
  }
